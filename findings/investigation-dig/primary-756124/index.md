...
| E-030 | 2026-05-02 | NARA Catalog API probe: `GET https://catalog.archives.gov/api/v1/search?q=US%203647970&rows=5` with `Accept: application/json`; `GET https://catalog.archives.gov/api/v1?naIds=302050` with same Accept header | Secondary (access-path verification) | High for HTTP observation in this environment | Neutral | retrieval-method status | Response body is the catalog SPA HTML shell (not JSON record payloads), matching the E-012 finding for `/api/v2/records/search`. Machine-readable catalog pull from this workspace remains blocked without a browser session or alternate auth path; continue NARA Kansas City email inquiry per `ARCHIVAL_TARGETS_756124.md` target (c). |
| E-031 | 2026-05-06 | NARA Catalog API authentication — tutorial `https://www.archives.gov/research/catalog/help/api-getting-started` + `curl` probes of `/api/v2/records/search` without `x-api-key` | Primary (custodian API instructions) / Secondary (HTTP observation) | High | Neutral | retrieval-method status | Official docs: `x-api-key` from Catalog engineers; `Catalog_API@nara.gov`. Without key, probes return **`text/html`** **~5454-byte** SPA shell for `q=302050`, `q=naId:302050`, `q=3647970`, and v1 paths. Refines E-030/E-012. Detail: `NARA_CATALOG_API_AUTH_LAYER5.md`. |

### Effect of E-017 through E-031 on the ledger
...
- The **archival custody chain** is now explicit (D-1 files via NC1-241-76-01; broader patent case files via N1-241-10-001; USPTO certified-copy / file-wrapper channel; conditional NSA RG 457 channel via N1-457-07-001). **E-030** extends the NARA machine-access negative result to the v1 catalog endpoints (SPA HTML, not JSON). **E-031** records that **v2** search is **documented to require** **`x-api-key`**; unauthenticated `curl` stays non-JSON in this workspace.
...
2. **Group 2200 successor retention (Layer-4 aligned).** N1-241-96-003 item 37 is event-driven to USPTO-internal custody (no closure interval on that entry). N1-241-10-001 crosswalks 37a/b into **item 2** (granted patent case files: **permanent**, transfer to NARA after closure) and **item 4.1** (evidentiary applications: **temporary**). Serial 756,124 issued; controlling bucket is **item 2**. Closure timing: **40 years after closure** for item 2, with the printed digit on the crosswalk row partially obscured on the archival PDF (reported by analogy with sibling rows; see `SF115_PRIMARY_READ_LAYER4.md`, E-A18 through E-A20). The retired "18 years" OCR line is not in either SF-115. **D-1**, **AC-AK**, **certified-copy logs**, and **ASPAB** files still had short destruction schedules unless copied into the file wrapper or retained elsewhere. (E-A16, E-A18 through E-A20; target (h) in `ARCHIVAL_TARGETS_756124.md`.)
...
5. **NARA Catalog API follow-up (E-030, refined E-031).** A 2026-05-02 curl probe of `catalog.archives.gov` v1 search and v1-by-naId endpoints with `Accept: application/json` still returns the single-page-app HTML shell rather than JSON payloads, consistent with E-012. A **2026-05-06** pass adds official NARA documentation: **`/api/v2/records/search` requires an `x-api-key`** header (key from Catalog engineers; `Catalog_API@nara.gov`). Repeating v2 and v1 paths **without** that key still yields the **~5454-byte** SPA HTML. No new accession metadata was retrieved from this workspace; archival closure remains on human NARA / USPTO channels already listed in `ARCHIVAL_TARGETS_756124.md`, **or** on future authenticated Catalog API access if a key is obtained.
